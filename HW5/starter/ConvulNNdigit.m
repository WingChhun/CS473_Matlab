...
figure;
for ff=1:length(convnet.Layers(2).Weights(1,1,1,:))
%    subplot(5,4,ff); pcolor(squeeze(convnet.Layers(2).Weights(:,:,1,ff))); colorbar
    subplot(5,4,ff); image(squeeze(convnet.Layers(2).Weights(:,:,1,ff)), 'CDataMapping', 'scaled'); colorbar
end

W8sMatrix=[];
for ff=1:length(convnet.Layers(2).Weights(1,1,1,:))
    tmp=squeeze(convnet.Layers(2).Weights(:,:,1,ff));
    W8sMatrix(:,ff)=tmp(:);


end
[v,d]=eig(W8sMatrix'*W8sMatrix);
figure; plot(diag(d))